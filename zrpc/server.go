...
import (
	...
	"github.com/anyinone/go-zero/zrpc/internal"
	"github.com/anyinone/go-zero/zrpc/internal/auth"
	"github.com/anyinone/go-zero/zrpc/internal/serverinterceptors"
...
// Start starts the RpcServer with Listener.
// Graceful shutdown is enabled by default.
// Use proc.SetTimeToForceQuit to customize the graceful shutdown period.
func (rs *RpcServer) StartWithListener(ls net.Listener) {
	if err := rs.server.StartWithListener(ls, rs.register); err != nil {
		logx.Error(err)
		panic(err)
	}
}
